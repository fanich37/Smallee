...
;(function (root, factory) {
	...
	Smallee.prototype.setStylesToTheElements = function () {
		...
		this.slides.forEach(function (item) {
			...
			item.style.width = sliderWidth / _this3.settings.slidesToShow + 'px';
		});
	};
	...
	Smallee.prototype.moveSlides = function (event) {
		var stepRange = this.defineTheStepRange();
		var wasMovedOn = this.defineSmalleeWasTranslatedOn();
		var limit = this.defineTheLimit();

		var nextStep = wasMovedOn - stepRange >= limit ? wasMovedOn - stepRange : limit;
		var prevStep = wasMovedOn + stepRange > 0 ? 0 : wasMovedOn + stepRange;
	...
	Smallee.prototype.dragSlides = function (isDown, isMoving, startPoint) {
		...
		if (isDown && isMoving) {
			if (event.clientX < startPoint) {
				var nextStep = wasMovedOn + (event.clientX - startPoint);
				console.log(nextStep);
				// this.inner.style.transform = `translate3d(${nextStep}px, 0, 0)`;
				// this.setControlsState();
			} else {
				var _nextStep = wasMovedOn - (startPoint - event.clientX);
			}
		}
	};
